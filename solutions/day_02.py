...
def task_1(games):
    ...
    return sum(int(game_id) for game_id, game_content in games.items() if valid_game(game_content))
...
def task_2(games):
    ...
    return sum(_calculate_power_set_cubes(game_content) for _, game_content in games.items())
